...
List<int> max_num = [192,49,502,214,534,4,762,422,900,405,1109,1233,1341,3];//教室番号の最大値
...
int search(cn){
  if(cn == null) return d;
  else if(cn.length == 0)return d;//cn=class number
  else if(cn.length<4){
    if(cn[0] == 'E'){
      try{//文字列から数値への変換を試す
        int i = int.parse(cn.substring(1));
        if(min_num[1]<= i && i <= max_num[1])return 1;//教室番号の最大値と最小値の間ならあるという判定(もちろん不正確)
        else return e;
    ...
    try {
      ...
      if(i == 2 || i == 3 || i == 4 || i == 6 || i == 8 || i == 0)return e;
        ...
        try{
          int j = int.parse(cn);
          if(min_num[i-1] <= j && j <= max_num[i-1])return i-1;
          else return e;
        }catch(exception){
          return e;
        }
    ...
    if(cn == "18号館ホール")return 17;
      ...
      try {
        ...
        if(min_num[7] <= i && i <= max_num[7])return 7;
        else return e;
      }catch(exception) {
        return e;
      }
    ...
    try{
      ...
      if(n == 10){
        ...
        if(min_num[n-1] <= i && i <= max_num[n-1])return n-1;
        else return e;
      }
      else return e;
    }catch(exception){
      if(cn == "音楽練習室" || cn == "舞台芸術実習室") return 5;
        ...
        try{
          ...
          if(min_num[13] <= i && i <= max_num[13])return 5;
          else return e;
        }catch(exception){
          return e;
        }
    ...
    try {
      ...
      if (9 < d && d < 14) {
        try {
          int i = int.parse(cn);
          if (min_num[d - 1] <= i && i <= max_num[d - 1])
            return d - 1;
          else
            return e;
        } catch (exception) {
